...
export default function Home() {
    ...
    const getLinks = webpages.map((webpage, index) => {
        ...
        return (
            <Link className='group transition duration-200 ease-in-out flex text-black items-center justify-center rounded bg-[#ffe0e9] hover:bg-[#eb8faf] h-60' to={location} key={index}>
                <div>
                    <img className='mx-auto w-[75px] h-[75px] transition duration-150 ease-in-out group-hover:scale-105' src={src} alt={webpage.webpage} />
                    <hr className='m-2 border-0 bg-gray-300 h-px' />
                    <p className='transition duration-150 ease-in-out group-hover:scale-105'>{webpage.webpage}</p>
                    <p className='transition duration-150 ease-in-out group-hover:scale-105'>{webpage.description}</p>
                </div>
            </Link>
        );
    });
    ...
    const getVideos = webpages.map((webpage, index) => {
        ...
        return (
            <div className='flex text-black items-center justify-center rounded bg-[#ffe0e9] p-5' key={index}>
                <div>
                    <p className='text-4xl font-medium'>{webpage.webpage.toUpperCase()}</p>
                    <hr className='m-2 border-0 bg-gray-300 h-px' />
                    <video className='w-[650px] h-[500px]' controls>
